...
export const fetchPoints = createAsyncThunk(
  ...
  async (pageble?: PageRequest) => {
    try {
      const res = pageble
        ? await findAllPontuacoes(pageble)
        : await findAllPontuacoes();
  ...
  extraReducers: builder => {
    ...
    builder.addCase(fetchPoints.fulfilled, (state, action) => {
      ...
      if (!action.payload.page.empty) {
        state.pontuacoes = action.payload.page.first
          ? action.payload.pontuacoes
          : [...state.pontuacoes!, ...action.payload.pontuacoes];
      }
    });
    ...
    builder.addCase(
      fetchPoints.rejected || fetchPointUsu.rejected,
      (state, action) => {
        state.loading = false;
